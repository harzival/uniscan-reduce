...
if "--" in sys.argv:
    ...
    parser.add_argument(
        "input",
        type=str,
        help="Path to the directory containing the lod directories (which contains the obj files)",
    )

    parser.add_argument(
        "output",
        type=str,
        help="Path to the directory where the output will be created",
    )
    parser.add_argument(
        "-rp",
        "--reduction-percentage",
        dest="reduction_percentage",
        type=float,
        required=False,
        default=0.05,
        help="The percentage of the original mesh's faces to try and use as the target face count for the new mesh.",
    )
    parser.add_argument(
        "-me",
        "--max-error",
        ...
        required=False,
        default=0.0015,
        help="The max Hausdorff error value that allows a mesh to pass the deformation test, instead of being remeshed with aditional faces.",
    )
    parser.add_argument(
        "-afp",
        "--add-faces-percentage",
        dest="add_faces_percentage",
        type=float,
        required=False,
        default=0.05,
        help="When a mesh does not pass the deform test, the target number of faces to remesh to is multiplied by this percentage, and added to the original target to get a new target face count to remesh with",
    )
    args = parser.parse_known_args(argv)[0]

hausdorff_script = Path(".") / "geomerror.mlx"
input_dir = Path(args.input)
output_dir = Path(args.output)
add_faces_percentage = args.add_faces_percentage
...
def main():
            ...
            for line in obj_path.open("r"):
                if line.startswith("mtllib "):
                    local_texture_path = Path(
                        ...
                        .replace(".mtl", ".jpg")
                        .rstrip("\n"))
                    texture_path = lod_dir / str(local_texture_path)
    ...
    for input_mesh in input_mesh_list:
        ...
        target_face_count = (
            round(get_total_face_count(mesh) * reduction_percentage)
            + boundary_face_count
        )
        ...
        while reduction_incomplete:
            ...
            if hausdorff_error < max_error:
                ...
                target_face_count += round(
                    target_face_count * add_faces_percentage
                )
                print(
